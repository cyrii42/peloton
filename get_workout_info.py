...
def get_new_workouts_num(df: pd.DataFrame) -> int:
    total_workouts = py_conn.GetMe()["total_workouts"]
    existing_workouts = df.shape[0]
...
def get_workout_dict(new_workouts: int) -> pd.DataFrame:
    peloton_dict = {
    'start_time_iso': [],
    'start_time_local': [],
    'instructor': [],
