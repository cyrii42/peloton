...
def process_workouts(
    py_conn: pylotoncycle.PylotonCycle, workouts_num: int = None
) -> pd.DataFrame:
    ride_group_list = []

    workouts = py_conn.GetRecentWorkouts(
        workouts_num
    )  ## defaults to all workouts if nothing passed

    for w in workouts:
        df_workout = pd.json_normalize(w)
        print(type(w["ride"]))
        df_workout_ride = pd.json_normalize(w["ride"])
        print(df_workout_ride)
        ...
        ride_attributes_dict.update(
            {column: df_workout[column][0] for column in df_workout.columns}
        )
        ride_attributes_dict.update(
            {
                f"ride_{column}": df_workout_ride[column][0]
        ...
        ride_attributes_dict.update(
            {
                df_summaries["slug"][index]: df_summaries["value"][index]
        ...
        ride_attributes_dict.update(
            {f"{index}_avg": value for index, value in df_average_values.items()}
        )
        ride_attributes_dict.update(
            {f"{index}_max": value for index, value in df_max_values.items()}
        )

        # Set "Strive Score" attribute & loop through HR Zone Duration columns
        if "effort_zones" in workout_metrics_by_id_dict.keys():
            df_effort_zones = pd.json_normalize(
                workout_metrics_by_id_dict["effort_zones"]
            )
            df_hr_zone_durations = pd.json_normalize(
                workout_metrics_by_id_dict["effort_zones"]["heart_rate_zone_durations"]
            )

            ride_attributes_dict.update(
                {"strive_score": df_effort_zones["total_effort_points"][0]}
            )
            ride_attributes_dict.update(
                {
                    column: df_hr_zone_durations[column][0]
...
def calculate_new_workouts_num(
    py_conn: pylotoncycle.PylotonCycle, df_input: pd.DataFrame
) -> int:
