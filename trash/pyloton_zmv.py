...
from pyloton_schema import PelotonWorkoutData, PelotonHumanInstructor
from peloton_json import PelotonJSONWriter
...
class PylotonZMV():
    def __init__(self, username: str = PELOTON_USERNAME, password: str = PELOTON_PASSWORD) -> None:
        self.py_conn = PylotonZMVConnector(username, password)
        self.total_workouts = None

    def get_total_workouts(self) -> int:
        try:
            resp = self.py_conn.session.get(f"{BASE_URL}/api/me", timeout=10)
            resp.raise_for_status()
            return resp.json()["total_workouts"]
        except requests.HTTPError:
            self.py_conn.get_new_login_token()
            resp = self.py_conn.session.get(f"{BASE_URL}/api/me", timeout=10)
            resp.raise_for_status()
            return resp.json()["total_workouts"]
    ...
    def get_workout_ids(self, num_workouts: int = None) -> list[str]:
        ...
        for page in range(pages):
            ...
            try:
                resp = self.py_conn.session.get(url, timeout=10)
                resp.raise_for_status()
            except requests.HTTPError:
                self.py_conn.get_new_login_token()
                resp = self.py_conn.session.get(url, timeout=10)
    ...
    def get_all_instructors(self) -> dict:
        try:
            resp = self.py_conn.session.get(f"{BASE_URL}/api/instructor?page=0&limit=100", timeout=10)
            resp.raise_for_status()
            return resp.json()
        except requests.HTTPError:
            self.py_conn.get_new_login_token()
